...
export default async function DoctorTrendsPage() {
  ...
  const moderateCases = cases.filter((c) => c.dehydrationLevel === "SOME").length;
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <Button asChild variant="outline" className="gap-1.5 shrink-0">
          <Link href="/doctor/reports">Generate Full Report</Link>
        </Button>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div className="rounded-xl border bg-card p-5">
          ...
          <p className="text-xs text-muted-foreground mt-1">Confirmed by medical staff</p>
        </div>
        ...
        <div className="rounded-xl border bg-card p-5 space-y-4">
          ...
          <div className="space-y-3 pt-2">
            {Object.entries(districtStats).map(([district, stats]) => {
              const pct = totalCases > 0 ? Math.round((stats.total / totalCases) * 100) : 0;
              return (
