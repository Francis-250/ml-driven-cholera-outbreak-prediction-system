...
export async function uploadEnvironmentalData(input: UploadEnvironmentalInput) {
  ...
  await prisma.auditLog.create({
    data: {
      ...
      entityId: record.id,
      description: `Staff member ${session.user.name} uploaded environmental surveillance data for ${district} (Risk: ${riskCalculation.riskLevel}, Score: ${riskCalculation.riskScore}%).`,
      metadata: {
        district,
        waterSource,
        waterContaminationLevel: input.waterContaminationLevel,
...
export async function bulkUploadEnvironmentalData(records: UploadEnvironmentalInput[]) {
  ...
  await prisma.auditLog.create({
    data: {
      ...
      description: `Staff member ${session.user.name} bulk imported ${dataToInsert.length} environmental surveillance records via CSV dataset (${highRiskCount} high risk).`,
      metadata: {
