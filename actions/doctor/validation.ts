...
export async function validateDiseaseRecord(input: ValidateRecordInput) {
  ...
  const updateData: {
    ...
    validationNotes: input.validationNotes?.trim() || (input.decision === "VALIDATED" ? "Validated by clinician." : "Case dismissed following clinical review."),
  ...
  // Notify the community user
  await prisma.notification.create({
    data: {
      userId: record.userId,
      assessmentId: record.id,
      type: "DOCTOR_ALERT",
      title: input.decision === "VALIDATED" ? "Case validated by clinician" : "Case review completed",
      message: `Dr. ${session.user.name} has ${input.decision.toLowerCase()} your cholera symptom report: ${input.validationNotes || "Consultation complete."}`,
    },
  });
  ...
  await prisma.auditLog.create({
    data: {
      ...
      entity: "Assessment",
      entityId: record.id,
      description: `Doctor ${session.user.name} marked disease case #${record.id.slice(0, 8)} as ${input.decision} (${record.district}).`,
      metadata: {
